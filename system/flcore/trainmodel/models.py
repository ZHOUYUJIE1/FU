...
class BaseHeadSplit(nn.Module):
    ...
    def forward(self, x):
        out = self.base(x)
        # 修正：如果输出是[batch, 512, 1, 1]，需要flatten为[batch, 512]
        if out.dim() == 4 and out.shape[2] == 1 and out.shape[3] == 1:
            out = out.view(out.size(0), -1)
        out = self.head(out)
        return out


class FedAUHeadSplit(nn.Module):
    """Base + main head + local auxiliary unlearning head."""

    local_only_param_prefixes = ("head_ul.",)

    def __init__(self, base, head, head_ul):
        super(FedAUHeadSplit, self).__init__()

        self.base = base
        self.head = head
        self.head_ul = head_ul
    ...
    def forward_with_aux(self, x):
        out = self.forward_features(x)
        return self.head(out), self.head_ul(out)
