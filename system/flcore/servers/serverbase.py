...
class Server(object):
    def __init__(self, args, times):
        ...
        self.best_client_buffer_states = None
        self.best_client_local_param_states = None
    ...
    def select_clients(self):
        ...
        selected_clients = self._ensure_forced_client_selected(selected_clients)

        return selected_clients

    def _ensure_forced_client_selected(self, selected_clients, excluded_client_ids=None):
        forced_client_id = getattr(self, "force_include_client_id", None)
        if forced_client_id is None:
            return selected_clients
        if excluded_client_ids and forced_client_id in excluded_client_ids:
            return selected_clients
        if any(client.id == forced_client_id for client in selected_clients):
            return selected_clients

        forced_client = next(
            (client for client in self.clients if client.id == forced_client_id),
            None,
        )
        ...
        replace_idx = np.random.randint(0, len(selected_clients))
        selected_clients[replace_idx] = forced_client
        return selected_clients
    ...
    def _extract_local_param_state(self, model):
        local_prefixes = tuple(getattr(model, "local_only_param_prefixes", ()))
    ...
    def _cache_best_eval_snapshot(self, test_acc):
        if test_acc <= self.best_test_acc:
            return

        self.best_test_acc = test_acc
        self.best_eval_round = len(self.rs_test_acc) - 1
        self.best_model_state_dict = self._cpu_state_dict(self.global_model.state_dict())
        self.best_client_buffer_states = {
            client.id: self._extract_buffer_state(client.model)
            for client in self.clients
        }
        self.best_client_local_param_states = {
            client.id: self._extract_local_param_state(client.model)
            for client in self.clients
        }
    ...
    def receive_models(self):
        ...
        # 确保采样的客户端数量不超过实际选择的客户端数量
        num_active = min(num_active, len(self.selected_clients))
        if num_active <= 0:
            num_active = len(self.selected_clients)
    ...
    def aggregate_parameters(self):
        ...
        client_states = [m.state_dict() for m in self.uploaded_models]
        param_keys = {name for name, _ in self.global_model.named_parameters()}
        local_only_prefixes = tuple(getattr(self.global_model, "local_only_param_prefixes", ()))

        for key, tensor in global_state.items():
            if key in param_keys:
                if any(key.startswith(prefix) for prefix in local_only_prefixes):
                    global_state[key] = prev_global_state.get(key, global_state[key]).clone()
                    continue
