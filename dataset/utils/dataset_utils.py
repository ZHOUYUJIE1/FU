...
DEFAULT_ALPHA = 0.1  # for Dirichlet distribution. 100 for exdir
...
def set_partition_config(alpha_value=None, batch_size_value=None, train_ratio_value=None):
    """Update shared dataset partition hyper-parameters at runtime."""
    global alpha, batch_size, train_ratio

    if alpha_value is not None:
        alpha = float(alpha_value)
    if batch_size_value is not None:
        batch_size = int(batch_size_value)
    if train_ratio_value is not None:
...
def check(config_path, train_path, test_path, num_clients, niid=False,
    ...
    if os.path.exists(config_path):
        ...
        if config['num_clients'] == num_clients and \
            ...
            config.get('class_per_client') == class_per_client:
